...
name: khazix-writer
description: |
  数字生命卡兹克（Khazix）的公众号长文写作skill。当用户需要撰写公众号文章、写稿子、续写文章、根据素材产出长文时使用。
  
  ⚠️ CSS 执行铁律（2026-05-23）：**每次生成 HTML 前必须先读 `references/streambert-reference.html`**，不能凭记忆生成 CSS。历史已证明记忆有 6+ 处错误（h2 边框 / p margin / blockquote 颜色 / 引言块颜色全部记错）。正确 CSS 值必须从 reference 文件逐项读取，写作时不许跳过此步。
---
---
...
GitHub 黑马项目的质量铁律（6维度验收标准、7级CSS层级、封面/字数/作者/图片检查），参考 `references/zhiliGitHub-quality.md`。**写作前必读。**
mmx 封面生成（2026-05-24）：`mmx image generate` 可生成 AI 封面，prompt 须英文，尺寸 896×512（必须8倍数，高度最小512），参考 `references/mmx-cover.md`。
